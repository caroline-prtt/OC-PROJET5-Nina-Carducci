var mauGallery = {
    init: function (selector, options) {
      ...
      function createRowWrapper(element) {
        if (!element.querySelector(".gallery-items-row")) { // Modifier la classe ici
          var rowWrapper = document.createElement("div");
          rowWrapper.className = "gallery-items-row row"; // Modifier la classe ici
      ...
      function wrapItemInColumn(element, columns) {
        ...
        columnWrapper.className = "item-column mb-4 col-12 col-sm-6 col-md-4 col-lg-4 col-xl-4 " + columnClasses; // Modifier la classe ici
      ...
      function moveItemInRowWrapper(element) {
        element.parentNode.parentNode.querySelector(".gallery-items-row").appendChild(element); // Modifier la classe ici
      }
      ...
      function filterByTag(tag) {
        filteredImages = []; // Réinitialiser la liste des images filtrées
        ...
        document.querySelectorAll(".gallery-item").forEach(function (item) {
          ...
          if (tag === "all" || itemTag === tag) {
            ...
            filteredImages.push(item); // Ajouter l'image filtrée à la liste
          } else {
      ...
      function openLightBox(item, lightboxId) {
        ...
        currentIndex = filteredImages.indexOf(item); // Mettre à jour l'index actuel
  ...
  // Initialisation de la galerie au chargement du document
  ...
  // Fonction pour fermer la modale lorsqu'on clique en dehors
  ...
  // Écouter les clics en dehors de la modale
